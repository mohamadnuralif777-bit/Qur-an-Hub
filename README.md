...
2. **In Render dashboard**, click **New → Blueprint** and connect your repo. Render will read `render.yaml` and create two services automatically:
...
4. **Trigger a deploy** for both services. The frontend Docker build bakes `VITE_API_URL` in at build time — Render handles this automatically via the `ARG` in `frontend/Dockerfile`.

5. **Seed the database** by running the seed script once from your local machine (with your Atlas `MONGO_URL`):
   ```bash
   cd backend
   pip install -r requirements.txt
   MONGO_URL="mongodb+srv://..." python seed.py
   ```

6. Your website is live! Share the frontend URL.
